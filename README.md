...
> **ai_ver1 브랜치**: 데이터 수집부터 라벨링, DB 업로드까지 전 과정을 포괄하는 1차 완성 end-to-end 파이프라인입니다.
> 뉴스 기사 크롤링 → 피처 추출 (로컬 모델 + Groq + Gemini) → Supabase2 업로드까지의 전체 흐름이 통합되어 있으며,
> 실제 운영 환경에서 검증된 첫 번째 완전한 버전입니다.
