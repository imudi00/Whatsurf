...
def _build_logic_prompt(struct: dict) -> str:
    return f"""뉴스 기사의 핵심 논거 유형을 분류하세요.
...
def extract_logic(struct: dict) -> dict:
    """기사 구조체에서 논거 유형과 근거를 반환"""
    prompt = _build_logic_prompt(struct)
    raw = call_llm(prompt)
    result = parse_json_response(raw)

    if result.get("logic") not in LOGIC_LABELS:
        result["logic"] = "사실/정보 전달"  # 기본값 fallback
    return result


def get_logic_label(struct: dict) -> str:
    """논거 유형 레이블만 간단히 반환"""
    return extract_logic(struct)["logic"]
