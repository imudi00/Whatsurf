  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        // 2. 폴링 없이 한 번만 클러스터 요청
        const clusterData = await fetchAPI(`/api/queries/${currentQueryId}/clusters`);
        const clusters = clusterData?.data?.clusters || clusterData?.clusters || [];
