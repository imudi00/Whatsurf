...
async def create_query(request: QueryRequest, background_tasks: BackgroundTasks):
    try:
        # 최근 24시간 내 같은 키워드 검색 기록 확인
        recent_cutoff = (datetime.now() - timedelta(hours=24)).isoformat()
        
        existing = supabase.table("queries") \
            .select("id, query_text, requested_at") \
            .eq("query_text", request.query_text) \
            .gte("requested_at", recent_cutoff) \
            .order("id", desc=True) \
            .limit(1) \
            .execute()

        if existing.data:
            # 최근 캐시 있음 → 바로 반환, 백그라운드 작업 없음
            ...
            return {
                ...
                "data": {
                    ...
                    "message": "캐시된 분석 결과를 불러옵니다.",
                    "cached": True
                }
            }
        else:
            # 캐시 없음 또는 오래됨 → 새로 생성 후 분석 시작
